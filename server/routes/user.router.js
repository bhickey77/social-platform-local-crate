...
router.get('/info', rejectUnauthenticated, (req, res) => {
  verbose && console.log('In the route api/user/info', req.user);
  ...
  pool.query(queryText, [req.user.id])
    .then(response => {
      verbose && console.log('Back from the db in the get user info route: ', response.rows[0]);
    ...
    .catch(error => {
      verbose && console.log('Error with the get user info route: ', error);
      res.sendStatus(500)
    })
})
...
router.post('/register', (req, res, next) => {
  ...
  pool.query(partnerQuery, partnerValues)
    .then(response => {
      verbose && console.log(response.rows[0]);
      ...
      pool.query(personQuery, personValues)
        .then(response => {
          verbose && console.log(`successfully inserted into person: `, response);
          res.sendStatus(200);
        })
        .catch(error => {
          verbose && console.log(`error inserting into person: `, error);
          res.sendStatus(500);
        })
    })
    .catch(error => {
      verbose && console.log(`error with insert into partner: `, error);
      res.sendStatus(500);
    })
});
